...
- **API Platform 4** for the REST API. **Resources are Infrastructure DTOs** (`#[ApiResource]` on
  `src/Infrastructure/ApiPlatform/Resource/`); the **domain model is never decorated**. Read/write go
  through **State Providers/Processors** (`src/Infrastructure/ApiPlatform/State/`) that bridge to the
  Application/Domain via the command bus and repository ports.
...
- **Layout**: hexagonal (ports & adapters) — `src/Domain/` (pure PHP, zero framework),
  `src/Application/` (use cases), `src/Infrastructure/` (adapters, incl. API Platform + the Symfony
  `Kernel`). Dependencies point inward, never outward.
