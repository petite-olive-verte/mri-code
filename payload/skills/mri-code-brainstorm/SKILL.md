...
description: >-
  ...
  Analysis front of the mri pipeline. Invoked by /mri-code-brainstorm.
disable-model-invocation: true
---

# mri-code-brainstorm — facilitated brainstorming
...
## Output
`.mri_code/docs/<project>/brief.md` (the product brief, the handoff artifact). Optional session
trace: `.mri_code/docs/<project>/brainstorm-notes.md`.
...
of a specific idea, suggest `/mri-code-forge`; to deepen an output, `/mri-code-elicit`.)

## Tracking
- At the **start**: mark `brainstorm` `[~]` in `.mri_code/docs/<project>/progress.md` (create it if missing).
- At the **end**: `brainstorm` `[x] → brief.md`, point to the next step.

## End
Recap the brief, then: "**Next step → `/mri-code-forge`** (harden the idea) or **`/mri-code-design`** if
it's already solid."

---
**User input:** $ARGUMENTS

💡 **Suggested model:** Opus (ideation / framing) — see `.mri_code/models.md`.
